...
if(count($Filter) > 0) {
header("location: ".APP.'/anime?filter='.json_encode($Filter));
} else {
header("location: ".APP.'/anime');
}
}
...
$Where .= 'WHERE posts.anime = "1" AND posts.status = "1" AND ';
if($Filter['quality']) {
...
$Config['title']= str_replace('${title}', $SelectCategory['name'], get($Settings, "data.anime_category_title", "seo"));
$Config['header']   = $SelectCategory['name'].' '.__('Anime');
if(!$SelectCategory['description']) {
$Config['description']  = str_replace('${title}', $SelectCategory['name'], get($Settings, "data.anime_category_description", "seo"));
}else {
